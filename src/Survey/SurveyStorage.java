    ...
    public static void writeAllSurvey() {
        ...
        for (int i = 0; i < surveys.size(); ++i) {
            ...
            if (survey.getQuestions().isEmpty()) {
                System.out.println("Список вопрос пуст");
            }
            else {
                ...
                for (int j = 0; j < survey.getQuestions().size(); ++j) {
                    ...
                    String questionType = "";
                    if (survey.getQuestions().get(j).getQuestionType() == QuestionType.ONE_OF_THE_TWO) {
                        questionType = "выбор одного из двух";
                    } else if (survey.getQuestions().get(j).getQuestionType() == QuestionType.ONE_OF_SEVERAL) {
                        questionType = "выбор одного из нескольких";
                    } else if (survey.getQuestions().get(j).getQuestionType() == QuestionType.FREE_RESPONSE) {
